import admin from "firebase-admin";
import dotenv from "dotenv";

dotenv.config();

// Combine the private key parts
const privateKey = [
  process.env.FIREBASE_PRIVATE_KEY_PART1,
  process.env.FIREBASE_PRIVATE_KEY_PART2,
  process.env.FIREBASE_PRIVATE_KEY_PART3,
  process.env.FIREBASE_PRIVATE_KEY_PART4,
  process.env.FIREBASE_PRIVATE_KEY_PART5,
  process.env.FIREBASE_PRIVATE_KEY_PART6,
  process.env.FIREBASE_PRIVATE_KEY_PART7,
  process.env.FIREBASE_PRIVATE_KEY_PART8,
  process.env.FIREBASE_PRIVATE_KEY_PART9,
  process.env.FIREBASE_PRIVATE_KEY_PART10,
  process.env.FIREBASE_PRIVATE_KEY_PART11,
  process.env.FIREBASE_PRIVATE_KEY_PART12,
  process.env.FIREBASE_PRIVATE_KEY_PART13,
  process.env.FIREBASE_PRIVATE_KEY_PART14,
  process.env.FIREBASE_PRIVATE_KEY_PART15,
  process.env.FIREBASE_PRIVATE_KEY_PART16,
  process.env.FIREBASE_PRIVATE_KEY_PART17,
  process.env.FIREBASE_PRIVATE_KEY_PART18,
  process.env.FIREBASE_PRIVATE_KEY_PART19,
  process.env.FIREBASE_PRIVATE_KEY_PART20,
  process.env.FIREBASE_PRIVATE_KEY_PART21,
  process.env.FIREBASE_PRIVATE_KEY_PART22,
  process.env.FIREBASE_PRIVATE_KEY_PART23,
  process.env.FIREBASE_PRIVATE_KEY_PART24,
  process.env.FIREBASE_PRIVATE_KEY_PART25,
  process.env.FIREBASE_PRIVATE_KEY_PART26,
  process.env.FIREBASE_PRIVATE_KEY_PART27,
  process.env.FIREBASE_PRIVATE_KEY_PART28
].join('\n');
 

export const firebaseCredential = admin.credential.cert({
  type: "service_account",
  project_id: "caltex-api",
  private_key_id: process.env.FIREBASE_PRIVATE_KEY_ID,
  private_key: privateKey,
  // private_key: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----',
 // private_key: "-----BEGIN PRIVATE KEY-----" + process.env.FIREBASE_PRIVATE_KEY.replace(/\Z/gm, "\n") + "-----END PRIVATE KEY-----",
  // private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----" ,
  client_email: "firebase-adminsdk-hocuy@caltex-api.iam.gserviceaccount.com",
  client_id: "113433279036650090499",
  auth_uri: "https://accounts.google.com/o/oauth2/auth",
  token_uri: "https://oauth2.googleapis.com/token",
  auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
  client_x509_cert_url:
    "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-hocuy%40caltex-api.iam.gserviceaccount.com",
  universe_domain: "googleapis.com",
});

admin.initializeApp({
  credential: firebaseCredential,
});
export const storage = admin.storage();

export default admin;
